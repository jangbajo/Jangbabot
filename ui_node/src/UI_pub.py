    ...
    def setupUI(self):
        ...
        for x in range(goods_count):
            if (int(goods_num[x + 1]) > 0):
                ...
                row += 1
                self.listorder = self.listorder + str(x+1)
...
class subThread(QObject):
    ...
    def run(self):
        ...
        while ((currentMode == AUTOMODE) and (n == '0')):
            if(currentMode != UIMODE):
                rospy.Subscriber("uimode", Int32, self.uimode_callback)
                rate.sleep()
...
#        self.finished.emit()
    ...
    def uimode_callback(self, data):
        global currentMode

        print("uimode sub get\n")

        if(data.data == UIMODE):
#            rospy.loginfo(rospy.get_caller_id() + " UIMODE")
            value = UIMODE
        elif(data.data == AUTOMODE):
            rospy.loginfo(rospy.get_caller_id() + " AUTOMODE")

#        currentMode = int(data.data)
...
#        self.thread = subThread(parent=self)
        self.obj = subThread()
        self.thread = QThread()

        self.obj.moveToThread(self.thread)
        self.obj.finished.connect(self.nextscreen)
...
class CompleteWindowClass(QMainWindow, complete_form_class):
    ...
    def __init__(self):
        ...
        print("CompleteWindowClass start\n")
        self.show()
...
if __name__ == "__main__":
    ...
    pub = rospy.Publisher('listorder',String, queue_size=10)
#    modepub = rospy.Publisher('mode',Int32, queue_size=10)
